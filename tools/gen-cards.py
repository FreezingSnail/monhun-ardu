...
FLAGS = 0
HEADER_SIZE = 8
# kind, pageMask, overlayCount + 4 u24 pages + 2x6 overlay + armor craft bill
# (u16 zenny + 2 x {itemIdx+1 u8, count u8}). Quests carry a zero bill.
ITEM_SIZE = 33
PAGE_MAX = 4
OVERLAY_MAX = 2
CRAFT_MAT_SLOTS = 2   # packed {itemIdx+1, count} pairs in the armor craft bill
ITEM_MAX = 32
...
def weapon_pages(node, by_id):
    """DESC / PARTS / STATS for one forge-tree node (ui.4, 5co.4). PARTS lists
    the active bill (the upgrade bill with `FROM <parent>` when the node has a
    parent, else the direct bill) plus a DIRECT line when the direct path is
    pricier; overlays carry the live HAVE counts for the required items."""
    name = node["label"]
    pages = []
    overlays = []
    ...
    img = new_card()
    draw_title(img, name, "DESC")
    draw_body(img, node["desc"] if node["desc"] else [name])
    pages.append((PAGE_DESC, img))

    if node["parent"] is not None:
        bill, mats, tag = node["cost"], node["mats"], "FROM " + by_id[node["parent"]]["label"]
    else:
        bill, mats, tag = node["directCost"], node["directMats"], "DIRECT"
    if bill > 0 or mats:
        img = new_card()
        draw_title(img, name, "PARTS")
        y = BODY_Y
        for mat in mats:
            label = mat["item"].replace("_", " ").upper()
            draw_have_line(img, y, label, mat["count"], 112)
            overlays.append({"kind": OVERLAY_HAVE, "page": PAGE_PARTS, "x": 112, "y": y,
                             "arg0": mat["itemIdx"], "arg1": 0})
            y += LINE_H
        draw_text(img, 4, y, "ZENNY %d" % bill, LIGHT)
        y += LINE_H
        draw_text(img, 4, y, tag, LIGHT)
        if node["parent"] is not None and node["directCost"] != node["cost"]:
            y += LINE_H
            draw_text(img, 4, y, "DIRECT %d" % node["directCost"], LIGHT)
        pages.append((PAGE_PARTS, img))

    img = new_card()
    draw_title(img, name, "STATS")
    draw_body(img, ["DMG %d%%" % node["dmgMul"], "SPD %d%%" % node["spdMul"]])
    pages.append((PAGE_STATS, img))
...
def compile_model(errors, root):
    ...
    for i, piece in enumerate(pieces):
        ...
        for page_id, _img in pages:
            mask |= 1 << page_id
        # Craft bill baked into the record (5co.6): the card A gates + debits it
        # itself, so the smith armor recipe read (screenRowArmorRecipe) is gone.
        craft = [{"item": mat["item"] + 1, "count": mat["count"]} for mat in piece["materials"][:CRAFT_MAT_SLOTS]]
        items.append({"kind": KIND_ARMOR, "index": i, "name": piece["id"],
                      "pages": pages, "overlays": overlays, "mask": mask,
                      "craftCost": piece["zenny"], "craftMats": craft})
    for i, quest in enumerate(quests):
        ...
        items.append({"kind": KIND_QUEST, "index": i, "name": quest["name"],
                      "pages": pages, "overlays": overlays, "mask": mask,
                      "craftCost": 0, "craftMats": []})

    # Forge-tree weapon cards (ui.4, 5co.4), appended after the quests. The card
    # index is WEAPON_BASE + node index; the bill lives on the node table (the
    # runtime reads mhForge), so the record's craft tail stays zero.
    nodes = load_forge_module().load_nodes(root)
    if not nodes:
        errors.add("data/forge", "missing or invalid forge tree (weapon cards)")
        return None
    by_id = {node["id"]: node for node in nodes}
    for node in nodes:
        pages, overlays = weapon_pages(node, by_id)
        mask = 0
        for page_id, _img in pages:
            mask |= 1 << page_id
        items.append({"kind": KIND_WEAPON, "index": node["index"], "name": node["id"],
                      "pages": pages, "overlays": overlays, "mask": mask,
                      "craftCost": 0, "craftMats": []})
...
KIND_NAMES = {KIND_ARMOR: "armor", KIND_QUEST: "quest", KIND_WEAPON: "weapon"}


def page_symbol(item, page_id):
    return "mh_card_%s_%s_%d" % (KIND_NAMES[item["kind"]], item["name"], page_id)
    ...
    for item in items:
        ...
        for i in range(OVERLAY_MAX):
            if i < len(item["overlays"]):
                ...
                blob += b"\x00" * 6
        mats = list(item["craftMats"])
        slots = []
        for i in range(CRAFT_MAT_SLOTS):
            if i < len(mats):
                slots += [mats[i]["item"], mats[i]["count"]]
            else:
                slots += [0, 0]
        blob += struct.pack("<HBBBB", item["craftCost"], slots[0], slots[1], slots[2], slots[3])
...
def emit_meta_header(items, fx_symbols):
    lines = []
    app = lines.append
    armor_count = sum(1 for item in items if item["kind"] == KIND_ARMOR)
    quest_base = armor_count
    weapon_base = armor_count + sum(1 for item in items if item["kind"] == KIND_QUEST)
    app("#pragma once")
    app("// Generated by tools/gen-cards.py -- do not edit.")
    app("//")
    app("// Prebaked detail-card ABI (bead monhun-ardu-5co.3, docs/ui-design.md):")
    app("// 8 B header + one fixed 33 B item record, little-endian, no padding.")
    app("// The trailing armor craft bill (5co.6) lets the card A gate + debit a")
    app("// craft itself; quest records carry a zero bill.")
    ...
    app("constexpr uint8_t KIND_WEAPON = %d;" % KIND_WEAPON)
    app("")
    ...
    app("")
    app("// Armor craft bill (5co.6): u16 zenny then CRAFT_MAT_SLOTS x")
    app("// (itemIdx+1 u8, count u8). Quests carry a zero bill.")
    app("constexpr uint8_t CRAFT_MAT_SLOTS = %d;" % CRAFT_MAT_SLOTS)
    app("constexpr uint8_t ITEM_CRAFT_OFF = 27;       // u16 zenny + mats")
    app("constexpr uint8_t ITEM_CRAFT_COST_OFF = 27;  // u16 zenny")
    app("constexpr uint8_t ITEM_CRAFT_MAT_OFF = 29;   // CRAFT_MAT_SLOTS x 2 B")
    app("constexpr uint8_t ITEM_CRAFT_MAT_STRIDE = 2;")
    app("")
    app("// Item indices + record offsets (armor in data/armor.json order, then")
    app("// quests sorted by (id, name) -- the generated quest_meta order -- then")
    app("// the forge-tree weapon nodes in data order).")
    app("constexpr uint8_t QUEST_BASE = %d;    // first quest card index" % quest_base)
    app("constexpr uint8_t WEAPON_BASE = %d;   // first weapon card index" % weapon_base)
    prefixes = {KIND_ARMOR: "CARD_ARMOR_", KIND_QUEST: "CARD_QUEST_", KIND_WEAPON: "CARD_WEAPON_"}
    for i, item in enumerate(items):
        tag = item["name"].upper()
        prefix = prefixes[item["kind"]]
...
def dump_model(items):
    for item in items:
        ...
        mats = " ".join("mat%d x%d" % (m["item"], m["count"]) for m in item["craftMats"])
        print("card %s %s: mask 0x%02X pages [%s] overlays [%s] craft %d [%s]"
              % (KIND_NAMES[item["kind"]], item["name"],
                 item["mask"], pages, overlays, item["craftCost"], mats))
