...
Blob layout (little-endian, explicit u8/u16, no padding, fixed section order):
    ...
    profile    26 B  engageDist, keepDist, attackDist, circleNum, circleDen,
...
PROFILE_OPTIONAL = {"staggerMax", "staggerDecay", "staggerRecoverT", "faceHold", "turnRate", "restAfter", "restT"}
...
def normalize_profile(errors, ctx, obj):
    ...
    return {
        ...
        # dzr: optional chain-gated stationary rest. restAfter = completed attacks
        # before a rest (0 = disabled, shipped default); restT = rest length in
        # ticks. Only the chicken authors it (2 / 110).
        "restAfter": read_int(errors, ctx, obj, "restAfter", 0, 255, default=0),
        "restT": read_int(errors, ctx, obj, "restT", 0, 255, default=0),
    }
...
def pack_model(errors, model):
    ...
    for entry in layout["creatures"]:
        ...
        record("PROFILE", b"".join([
            ...
            u8(profile["restAfter"]), u8(profile["restT"]),
        ]))
...
def emit_data_header(model, compiled):
    ...
    app("    uint8_t restAfter, restT;   // chain-gated stationary rest (0 = disabled, dzr)")
    app("};")
    app("")
    app("struct Skeleton {")
        ...
        if section == "CREATURES":
            ...
            for entry in layout["creatures"]:
                ...
                app("    {%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d}," % (
                    ...
                    profile["restAfter"], profile["restT"]))
...
def emit_expect_header(model, compiled):
    ...
    for creature in model["creatures"]:
        ...
        app("constexpr uint8_t PROFILE_%s_REST_AFTER = %d;" % (cid, creature["profile"]["restAfter"]))
        app("constexpr uint8_t PROFILE_%s_REST_T = %d;" % (cid, creature["profile"]["restT"]))
        if creature["attacks"]:
    ...
    for creature in model["creatures"]:
        ...
        if art["sheet"]:
            print("  art: sheet%d anchorY%d stride%d idle%d+%d windup%d attack%d recover%d flash%d dead%d" % (
                art["sheet"], art["anchorY"], art["stride"], art["idle0"], art["idleCount"],
                art["windup"], art["attack"], art["recover"], art["flash"], art["dead"]))
        print("  profile: engage%d keep%d attack%d circle%d/%d retreat%d/%d stagger%d/%d/%d faceHold%d turnRate%d cd%d+%d spawn%d/%d stun%d rest%d/%d" % (
            profile["engageDist"], profile["keepDist"], profile["attackDist"],
            profile["circleNum"], profile["circleDen"], profile["retreatNum"], profile["retreatDen"],
