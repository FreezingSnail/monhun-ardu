...
static uint16_t hashState(const Game& g) {
  ...
  h = mix(h, p.state); h = mix(h, p.t); h = mix(h, p.hitDone ? 1 : 0);
  h = mix(h, p.chain); h = mix(h, p.chainWin); h = mix(h, p.aBuffer);
  h = mix(h, p.stance); h = mix(h, p.stanceT); h = mix(h, p.stanceAuto);
  h = mix(h, p.whirlTick); h = mix(h, p.throwCd); h = mix(h, p.riposteT);
  h = mix(h, p.bHeld); h = mix(h, p.bReady ? 1 : 0); h = mix(h, p.bLocked ? 1 : 0);
  h = mix(h, p.iT); h = mix(h, p.shell); h = mix(h, p.reload);
  h = mix(h, p.shells[0]); h = mix(h, p.shells[1]);
  int32_t aId = 0, aSt = 0, aAc = 0, aRc = 0, aDm = 0, aRe = 0, aSm = 0;
  if (p.atk) {
    aId = attackId(p.atk); aSt = attackStartup(p.atk); aAc = attackActive(p.atk);
    aRc = attackRecover(p.atk); aDm = attackDmg(p.atk); aRe = attackReach(p.atk);
    aSm = attackStam(p.atk);
  }
  h = mix(h, aId); h = mix(h, aSt); h = mix(h, aAc); h = mix(h, aRc);
  h = mix(h, aDm); h = mix(h, aRe); h = mix(h, aSm);

  // monster
  h = mix(h, m.x); h = mix(h, m.y); h = mix(h, m.subX); h = mix(h, m.subY);
  h = mix(h, m.hp); h = mix(h, m.state); h = mix(h, m.t); h = mix(h, m.cd);
  h = mix(h, m.fx); h = mix(h, m.fy); h = mix(h, m.lvx); h = mix(h, m.lvy);
  ...
  if (m.atk) {
    mKd = monsterAttackKind(m.atk); mWu = monsterAttackWindup(m.atk);
    mAc = monsterAttackActive(m.atk); mRc = monsterAttackRecover(m.atk);
    mDm = monsterAttackDmg(m.atk); mRe = monsterAttackReach(m.atk);
    mHw = monsterAttackHw(m.atk); mHh = monsterAttackHh(m.atk);
  }
  h = mix(h, mKd); h = mix(h, mWu); h = mix(h, mAc); h = mix(h, mRc);
  h = mix(h, mDm); h = mix(h, mRe); h = mix(h, mHw); h = mix(h, mHh);
  ...
  for (int16_t i = 0; i < g.projN; i++) {
    ...
    h = mix(h, pr.x * 16 + pr.subX); h = mix(h, pr.y * 16 + pr.subY);
    h = mix(h, pr.vx); h = mix(h, pr.vy);
  ...
  for (int16_t i = 0; i < g.fxN; i++) {
    ...
    h = mix(h, e.crit ? 1 : 0); h = mix(h, e.text);
  }
...
static int32_t snapField(const Game& g, uint8_t f) {
  ...
  switch (f) {
    ...
    case 1: return p.y;
    case 2: return p.hp;
    case 3: return p.stam;
    case 4: return p.state;
    case 5: return p.stance;
    case 6: return p.chain;
    case 7: return p.shells[0];
    case 8: return p.shells[1];
    case 9: return p.reload;
    case 10: return g.projN;
    case 11: return g.train.total;
    case 12: return g.train.last;
    case 13: return m.x;
    case 14: return m.y;
    case 15: return m.state;
    case 16: return m.hp;
    case 17: return m.stun;
    case 18: return g.camX;
    case 19: return g.camY;
    default: return 0;
  }
}
...
static void parCheck(FxTest& test, int32_t got, int32_t want, uint16_t scene,
                     const __FlashStringHelper* what) {
  if (got == want) { ++test.passCount; return; }
  ++test.failCount;
...
static void dumpState(const Game& g) {
  Serial.print(F("  p("));
  Serial.print(g.player.x); Serial.print(',');
  Serial.print(g.player.y); Serial.print(F(") st="));
  Serial.print(g.player.state); Serial.print(F(" hp="));
  Serial.print(g.player.hp); Serial.print(F(" sm="));
  Serial.print(g.player.stam); Serial.print(F(" frz="));
  Serial.print(g.freeze); Serial.print(F(" pn="));
  Serial.print(g.projN);
  Serial.print(F(" m("));
  Serial.print(g.monster.x); Serial.print(',');
  Serial.print(g.monster.y); Serial.print(F(") ms="));
  Serial.print(g.monster.state); Serial.print(F(" mhp="));
    ...
    for (uint16_t t = 0; t < n; t++) {
      ...
      if (tick % parity_fx::CP_STRIDE == 0 || tick == n) {
        ...
        for (uint8_t f = 0; f < parity_fx::CP_FIELDS; f++) {
          parCheck(test, snapField(g, f),
                   mhPgmReadI16(&parity_fx::snaps[sb + f]), s,
                   F("snap"));
        }
        ++cp;
      }
