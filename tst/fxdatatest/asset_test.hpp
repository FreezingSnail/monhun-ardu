...
inline void test_assets(FxTest &test) {
    ...
    // Breakable pole variants (beads monhun-ardu-6zb.5 / 6zb.7): SEVER/CRACK
    // 20x40, BREAK 28x40. Each variant now ships SIX frames in stage*2 + flash
    ...
    test.expectEq(poleFrames(fxpole_break, fxscatter, 28), 6, F("break 6 frames"));
