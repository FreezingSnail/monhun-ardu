...
struct Rec {
  ...
  void reset() {
    hits = 0; lastDmg = 0; lastPush = 0; lastEffect = 0;
    shoves = 0; stuns = 0; lastFreeze = 0;
  }
};
Rec rec;

void onHit(Game&, int dmg, int, int, int push, int effect) {
  rec.hits++; rec.lastDmg = dmg; rec.lastPush = push; rec.lastEffect = effect;
}
void onShove(Game&, int, int, int, int freeze) { rec.shoves++; rec.lastFreeze = freeze; }
void onStun(Game&, int) { rec.stuns++; }
...
void stepN(Game& g, int n, Input in = Input{ 0, 0, false, false }) {
  for (int i = 0; i < n; i++) stepPlayer(g, in);
}
...
void attackOnce(Game& g) {
  ...
  for (int i = 0; i < 40 && g.player.state != PS_IDLE; i++) stepN(g, 1);
}
