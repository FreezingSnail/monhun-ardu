...
void holdGuard(Game& g) {
  for (int i = 0; i < HOLD_TICKS + 2; i++) stepWorld(g, HOLD_B);
}

// one A press while B (guard) is held; the shell fires this tick
void fireA(Game& g) { stepWorld(g, Input{ 0, 0, true, true }); }

// release A, keep holding B / guard
void releaseA(Game& g) { stepWorld(g, Input{ 0, 0, false, true }); }

void idleGuard(Game& g, int n) {
  for (int i = 0; i < n; i++) stepWorld(g, HOLD_B);
}

void idleTicks(Game& g, int n) {
  for (int i = 0; i < n; i++) stepWorld(g, IDLE);
}
...
void armShot(Game& g, int8_t shot, int16_t cx, int16_t cy, int16_t fx,
             int16_t fy) {
  g.lastShot = shot;
  g.lastShotX = cx; g.lastShotY = cy;
  g.lastShotFx = fx; g.lastShotFy = fy;
  spawnShot(g);
}
...
void ShellSuite(TestRunner& runner) {
...

  {
    ...
    for (int i = 0; i < 25; i++) updateEffects(g);
...

  {
    ...
    initGame(g, W_SWORD);
    initWorld(g, MODE_TRAIN);
    g.tick = 100; damagePole(g, 100, 100, 100); // body -> 100
    g.tick = 200; damagePole(g, 50, 100, 100);  // body -> 50
    g.tick = 500;
    ...
    g.tick = 800; damagePole(g, 7, 100, 100); // 7 -> rounds to 1
    g.tick = 801;
...

  {
    ...
    initMonster(g);
    initWorld(g, MODE_TRAIN);
    g.pole.rect = Rect{ static_cast<int16_t>(g.player.x + 20),
                        g.player.y, 20, 36 };
    const int16_t mx = g.monster.x, my = g.monster.y;
    stepWorld(g, Input{ 0, 0, true, false });
    for (int i = 0; i < 20 && g.player.state != PS_IDLE; i++) idleTicks(g, 1);
...

  {
    ...
    stepWorld(g, Input{ 0, 0, false, false });
    for (int i = 0; i < 40 && g.player.state != PS_IDLE; i++) idleTicks(g, 1);
