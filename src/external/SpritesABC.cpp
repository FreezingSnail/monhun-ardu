...
static void SpritesABC::drawFX(int16_t x, int16_t y, uint24_t image, uint8_t mode, uint16_t frame) {
    asm volatile(R"ASM(
        ...
        L%=_delay_7:
            ret
                 ...
                 :
                 : [fxport] "I"(_SFR_IO_ADDR(FX_PORT)), [fxbit] "I"(FX_BIT), [spdr] "I"(_SFR_IO_ADDR(SPDR)), [spsr] "I"(_SFR_IO_ADDR(SPSR)), [sreg] "I"(_SFR_IO_ADDR(SREG)),
                   [page] "i"(&FX::programDataPage), [drawf] ""(SpritesABC::drawSizedFX));
}

[[gnu::naked, gnu::noinline]]
static void SpritesABC::drawSizedFX(int16_t x, int16_t y, uint8_t w, uint8_t h, uint24_t image, uint8_t mode, uint16_t frame) {
    asm volatile(R"ASM(
...
[[gnu::naked, gnu::noinline]]
void SpritesABC::drawBasicFX(int16_t x, int16_t y, uint8_t w, uint8_t h, uint24_t image, uint8_t mode) {
    /*
    ...
    asm volatile(R"ASM(
        ...
        L%=_end_postpop:
            ...
            ret
                 ...
                 :
                 : [sBuffer] "i"(Arduboy2Base::sBuffer), [fxport] "I"(_SFR_IO_ADDR(FX_PORT)), [fxbit] "I"(FX_BIT), [spdr] "I"(_SFR_IO_ADDR(SPDR)), [spsr] "I"(_SFR_IO_ADDR(SPSR)),
                   [sreg] "I"(_SFR_IO_ADDR(SREG)), [page] "i"(&FX::programDataPage)
...
void SpritesABC::fillRect(int16_t x, int16_t y, uint8_t w, uint8_t h, uint8_t color) {
    ...
    asm volatile(R"ASM(
        ...
        L%=_finish:
                 ...
                 : [buf] "+&e"(buf), [w] "+&d"(w), [rows] "+&r"(rows), [col] "=&d"(col)
