...
struct SpritesU {
    ...
    static void drawOverwrite(int16_t x, int16_t y, uint8_t const *image, uint16_t frame);
    static void drawOverwrite(int16_t x, int16_t y, uint8_t w, uint8_t h, uint8_t const *image);
#endif

#ifdef SPRITESU_OVERWRITE
    static void drawPlusMask(int16_t x, int16_t y, uint8_t const *image, uint16_t frame);
    static void drawPlusMask(int16_t x, int16_t y, uint8_t w, uint8_t h, uint8_t const *image);
#endif

#ifdef SPRITESU_FX
    static void drawOverwriteFX(int16_t x, int16_t y, uint24_t image, uint16_t frame);
    static void drawOverwriteFX(int16_t x, int16_t y, uint8_t w, uint8_t h, uint24_t image, uint16_t frame);
    static void drawPlusMaskFX(int16_t x, int16_t y, uint24_t image, uint16_t frame);
    static void drawPlusMaskFX(int16_t x, int16_t y, uint8_t w, uint8_t h, uint24_t image, uint16_t frame);
#endif
    ...
    static void drawBasic(int16_t x, int16_t y, uint8_t w, uint8_t h, uint24_t image, uint16_t frame, uint8_t mode);
    static void drawBasicNoChecks(uint16_t w_and_h, uint24_t image, uint8_t mode, int16_t x, int16_t y);
};
...
void SpritesU::drawBasic(int16_t x, int16_t y, uint8_t w, uint8_t h, uint24_t image, uint16_t frame, uint8_t mode) {
    if (x >= 128)
        return;
    if (y >= 64)
        return;
    ...
    */
    ...
    asm volatile(R"ASM(
        ...
        1:
                 ...
                 : [h] "+&r"(h), [image] "+&r"(image), [tmp] "=&r"(tmp)
                 : [frame] "r"(frame), [mode] "r"(mode), [w] "r"(w));
#else
    if (frame != 0) {
        h >>= 3;
        if (mode & 1)
...
void SpritesU::drawBasicNoChecks(uint16_t w_and_h, uint24_t image, uint8_t mode, int16_t x, int16_t y) {
    ...
    asm volatile(R"ASM(
        ...
        8:
            clr __zero_reg__
                 ...
                 : [pages] "+&r"(pages), [shift_coef] "=&d"(shift_coef), [shift_mask] "=&r"(shift_mask), [page_start] "=&a"(page_start),
                   [cols] "=&r"(cols), [col_start] "=&r"(col_start), [bottom] "=&r"(bottom), [buf] "+&r"(buf), [buf_adv] "=&a"(buf_adv),
                   [image_adv] "=&r"(image_adv), [x] "+&r"(x), [y] "+&r"(y), [image] "+&r"(image)
                 : [mode] "r"(mode), [w] "r"(w));
        ...
        asm volatile(R"ASM(
            ...
            L%=_finish:
                ...
                clr __zero_reg__
                     ...
                     : [buf] "+&x"(buf), [image] "+&z"(image_ptr), [pages] "+&r"(pages), [count] "=&r"(count), [buf_data] "=&r"(buf_data),
                       [image_data] "=&r"(image_data)
                     : [cols] "r"(cols), [buf_adv] "r"(buf_adv), [image_adv] "r"(image_adv), [shift_mask] "r"(shift_mask),
                       [shift_coef] "r"(shift_coef), [bottom] "r"(bottom), [page_start] "r"(page_start)
                     : "r28", "r29", "memory");
#else
        ...
        asm volatile(R"ASM(
            ...
            L%=_finish:
                ...
                clr __zero_reg__
                     ...
                     : [buf] "+&x"(buf), [image] "+&z"(image_ptr), [pages] "+&r"(pages), [count] "=&r"(count), [buf_data] "=&r"(buf_data),
                       [image_data] "=&r"(image_data), [mask_data] "=&r"(mask_data)
                     : [cols] "r"(cols), [buf_adv] "r"(buf_adv), [image_adv] "r"(image_adv), [shift_coef] "r"(shift_coef),
                       [bottom] "r"(bottom), [page_start] "r"(page_start)
                     : "r28", "r29", "memory");
#else
        // TODO: C implementation
#endif
        ...
        asm volatile(R"ASM(
            ...
            L%=_finish:
                ...
                in r0, %[spsr]
                     ...
                     : [buf] "+&x"(buf), [bufn] "=&z"(bufn), [image] "+&r"(image), [pages] "+&r"(pages), [count] "=&r"(count),
                       [buf_data] "=&r"(buf_data), [image_data] "=&r"(image_data), [reseek] "=&r"(reseek), [sfc_read] "=&d"(sfc_read)
                     : [cols] "r"(cols), [w] "r"(w), [buf_adv] "r"(buf_adv), [image_adv] "r"(image_adv), [shift_coef] "r"(shift_coef),
                       [shift_mask] "r"(shift_mask), [bottom] "r"(bottom), [page_start] "r"(page_start), [mode] "r"(mode),
                       [SFC_READ] "I"(SFC_READ), [fxport] "I"(_SFR_IO_ADDR(FX_PORT)), [fxbit] "I"(FX_BIT), [spdr] "I"(_SFR_IO_ADDR(SPDR)),
                       [spsr] "I"(_SFR_IO_ADDR(SPSR)), [spif] "I"(SPIF), [page] "i"(&FX::programDataPage)
                     : "memory");
#else
        // TODO: C implementation
#endif
...
#ifdef SPRITESU_OVERWRITE
void SpritesU::drawOverwrite(int16_t x, int16_t y, uint8_t const *image, uint16_t frame) {
    uint8_t w, h;
#if ARDUINO_ARCH_AVR
    asm volatile("lpm %[w], Z+\n"
                 "lpm %[h], Z+\n"
...
void SpritesU::drawOverwrite(int16_t x, int16_t y, uint8_t w, uint8_t h, uint8_t const *image) {
    drawBasic(x, y, w, h, (uint24_t)image, 0, MODE_OVERWRITE);
}
#endif

#ifdef SPRITESU_PLUSMASK
void SpritesU::drawPlusMask(int16_t x, int16_t y, uint8_t const *image, uint16_t frame) {
    uint8_t w, h;
#if ARDUINO_ARCH_AVR
    asm volatile("lpm %[w], Z+\n"
                 "lpm %[h], Z+\n"
...
void SpritesU::drawPlusMask(int16_t x, int16_t y, uint8_t w, uint8_t h, uint8_t const *image) {
...
void SpritesU::fillRect_i8(int8_t x, int8_t y, uint8_t w, uint8_t h, uint8_t color) {
    ...
    asm volatile("mul %[r0], %[c128]\n"
                 ...
                 "adc %B[buf], __zero_reg__\n"
                 : [buf] "+&e"(buf)
                 : [r0] "r"(r0), [x] "r"(xc), [c128] "r"((uint8_t)128));
#else
    buf += r0 * 128 + xc;
#endif
    ...
    uint8_t bot = c1;
    if (m0 == 0)
        ++rows;   // no top fragment
    if (bot == 0)
        ++rows;   // no bottom fragment
    c0 &= color;
    c1 &= color;
