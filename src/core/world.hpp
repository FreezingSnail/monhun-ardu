...
// resetHunt(g) — it is NOT the same as newGame() (which would reset the area to
// hunt).
...
static void stepGame(Game &g, const Input &inp) {
    ...
    tryHeal(g, bP);
    // Camp smithy interaction removed with the SMITH screen (ui.3.1, 5co.6);
    // the hub FORGE trees replace it in ui.4.
