...
static void initWorld(Game& g, int8_t mode) {
  ...
  g.projN = 0;
  for (int16_t i = 0; i < MAX_PROJECTILES; i++) g.proj[i] = Projectile{};
  g.fxN = 0;
  for (int16_t i = 0; i < MAX_EFFECTS; i++) g.fx[i] = Effect{};
  ...
  g.train.head = 0;
  g.train.count = 0;
  for (int16_t i = 0; i < MAX_TRAIN_EVENTS; i++) g.train.ev[i] = TrainEvent{};
  if (mode == MODE_TRAIN) syncPoleTarget(g);
}

// ---------------------------------------------------------------- effects

static void addEffect(Game& g, int16_t x, int16_t y, int16_t life, bool crit,
                      int16_t text) {
  if (g.fxN >= MAX_EFFECTS) { // device cap: drop oldest, keep newest
    for (int16_t i = 1; i < MAX_EFFECTS; i++) g.fx[i - 1] = g.fx[i];
    g.fxN = MAX_EFFECTS - 1;
  }
  Effect& e = g.fx[g.fxN++];
  e.x = x; e.y = y; e.t = 0; e.life = life; e.crit = crit; e.text = text;
}
...
static void updateEffects(Game& g) {
  for (int16_t i = g.fxN - 1; i >= 0; i--) {
    ...
    if (g.fx[i].t >= g.fx[i].life) {
      for (int16_t j = i + 1; j < g.fxN; j++) g.fx[j - 1] = g.fx[j];
      g.fxN--;
    }
  }
}
...
static void trainAdd(TrainStats& t, int32_t tick, int16_t dmg) {
  ...
  if (t.count < MAX_TRAIN_EVENTS) t.count++;
}
...
static int16_t trainDps(const Game& g) {
  ...
  for (int16_t i = 0; i < g.train.count; i++) {
    ...
    if (g.train.ev[idx].tick > cutoff) sum += g.train.ev[idx].dmg;
  }
...
static void damagePole(Game& g, int dmg, int hx, int hy) {
  ...
  addEffect(g, static_cast<int16_t>(hx), static_cast<int16_t>(hy - 6), 26, crit,
...
static void poleOnHit(Game& g, int dmg, int hx, int hy, int push, int effect) {
  (void)push; (void)effect;
  damagePole(g, dmg, hx, hy);
}
static void poleOnShove(Game&, int, int, int, int) {} // pole never moves
...
static void spawnShot(Game& g) {
  ...
  addEffect(g, static_cast<int16_t>(cx + ((fx * 10) >> 4)),
            static_cast<int16_t>(cy + ((fy * 10) >> 4)), 5, true, 0);
  ...
  if (shellPellets(sh) == 1) {
    ...
    dirX[0] = left.x;  dirY[0] = left.y;
    dirX[1] = fx;      dirY[1] = fy;
    dirX[2] = right.x; dirY[2] = right.y;
    n = 3;
  }

  for (int8_t i = 0; i < n; i++) {
    if (g.projN >= MAX_PROJECTILES) { // device cap: drop oldest
      for (int16_t j = 1; j < MAX_PROJECTILES; j++) g.proj[j - 1] = g.proj[j];
...
static void removeProjectile(Game& g, int16_t i) {
  for (int16_t j = i + 1; j < g.projN; j++) g.proj[j - 1] = g.proj[j];
  g.projN--;
}
  ...
  for (int16_t i = g.projN - 1; i >= 0; i--) {
    ...
    if (g.target.alive) {
      ...
      if (r.overlaps(g.target.rect)) {
        ...
        if (g.mode == MODE_TRAIN) damagePole(g, pr.dmg, hx, hy);
        else damageMonster(g, pr.dmg, hx, hy);
