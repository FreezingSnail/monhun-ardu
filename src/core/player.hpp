...
void Player::init(int8_t weapon) {
  ...
  subX = 0; subY = 0;
  vx = 0; vy = 0;
  fx = fp::FP; fy = 0; // face E
  hp = 100; hpMax = 100;
  stamMax = 100; stam = 100; stamSub = 0;
  state = PS_IDLE; t = 0; atk = nullptr; hitDone = false;
  chain = 0; chainWin = 0; aBuffer = 0;
  stance = ST_NONE; stanceT = 0; stanceAuto = 0; whirlTick = 0;
...
static void clampPlayer(Player& p) {
  ...
  if (p.y < 0) p.y = 0;
  if (p.y > WORLD_H - p.h) p.y = WORLD_H - p.h;
}
    ...
    if (p.whirlTick % 16 == 0) {
      if (g.target.alive) {
        ...
        if (circleRectOverlap(static_cast<int16_t>(cx), static_cast<int16_t>(cy), 24, g.target.rect)) {
          if (g.target.onHit) g.target.onHit(g, 8, cx, cy, 8, 0);
        }
      }
    }
...
static void playerHurt(Game& g, int16_t dmg, int16_t faceX, int16_t faceY) {
  ...
  if (p.state == PS_DEFLECT && p.t > 0) {
    if (g.target.onStun) g.target.onStun(g, 28);
  ...
  if (p.stance == ST_PARRY && p.stanceT <= 20) {
    p.riposteT = 90;
    if (g.target.onStun) g.target.onStun(g, 60);
...
static void updatePlayer(Game& g, const Input& inp, bool aP, bool bP, bool bR) {
  ...
  if (inp.b && p.bReady) {
    p.bHeld++;
    if (p.bHeld == HOLD_TICKS && p.stance == ST_NONE && !p.bLocked) enterStance(g, def);
  }
  if (bR) {
  ...
  if (p.stance != ST_NONE) updateStance(g, def);
  clampPlayer(p);
}
