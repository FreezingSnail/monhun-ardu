...
static inline bool mhAudioEnabled() { return true; }
...
static void audioPlay(uint8_t cue) {
  switch (cue) {
    case CUE_HIT:     ArduboyTones::tone(494, 22); break;
    case CUE_CRIT:    ArduboyTones::tone(659, 16, 1047, 40); break;
    case CUE_TRAIN:   ArduboyTones::tone(784, 24); break;
    case CUE_HURT:    ArduboyTones::tone(155, 45); break;
    case CUE_PARRY:   ArduboyTones::tone(880, 18, 1175, 55); break;
    case CUE_DEFLECT: ArduboyTones::tone(698, 18, 880, 34); break;
    case CUE_GUARD:   ArduboyTones::tone(196, 30, 262, 45); break;
    case CUE_WINDUP:  ArduboyTones::tone(175, 20, 233, 30); break;
    case CUE_SHOT:    ArduboyTones::tone(1568, 14, 1047, 24); break;
    case CUE_RELOAD:  ArduboyTones::tone(1047, 12, 1568, 28); break;
    default: break;
  }
}
#else
...
static void audioCue(AudioState& s, uint8_t cue) {
  if (cue == CUE_NONE) return;
  if (cue == s.lastCue && static_cast<int16_t>(s.tick - s.lastCueTick) < 2) return;
  s.lastCue = cue;
...
static void audioUpdate(AudioState& s, const Game& g) {
  ...
  if (!s.inited || g.tick < s.tick) { audioSnapshot(s, g); return; }
  ...
  for (int16_t i = 0; i < g.fxN; i++) {
    ...
    if (e.t != 1) continue;
    if (e.text) { if (e.crit) critText = true; }
    else if (e.crit) critSpark = true;
  }
  ...
  if (g.projN > s.projN) audioCue(s, CUE_SHOT);
  if (s.reload > 0 && p.reload == 0) audioCue(s, CUE_RELOAD);
  if (m.state == MS_WINDUP && s.monsterState != MS_WINDUP) audioCue(s, CUE_WINDUP);
