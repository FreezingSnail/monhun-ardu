...
//   y=36  TARGET LUNGE SWEEP HEAVY POLE (same)
//   y=54  A START                       (footer hint)
...
constexpr int16_t MENU_TARGET_Y = 36;
constexpr int16_t MENU_FOOTER_Y = 54;
...
static const char MH_PROGMEM MENU_TARGET_NAMES[] = "LUNGE\0SWEEP\0HEAVY\0POLE";
static const uint8_t MH_PROGMEM MENU_WEAPON_LEN[3] = {3, 3, 3};
static const uint8_t MH_PROGMEM MENU_WEAPON_OFF[3] = {0, 4, 8};
static const uint8_t MH_PROGMEM MENU_TARGET_LEN[4] = {5, 5, 5, 4};
static const uint8_t MH_PROGMEM MENU_TARGET_OFF[4] = {0, 6, 12, 18};
...
// fixed PROGMEM tables, so the row layout costs no RAM. This is device UI text,
// not FX content data (42n.1 policy covers sim content tables only).
static void menuOptionRow(const char MH_PROGMEM *names, const uint8_t MH_PROGMEM *offs, const uint8_t MH_PROGMEM *lens, uint8_t count, int8_t sel, int16_t y) {
...
static void drawMenu(const MenuState &m) {
    ...
    menuOptionRow(MENU_WEAPON_NAMES, MENU_WEAPON_OFF, MENU_WEAPON_LEN, 3, m.weapon, MENU_WEAPON_Y);

    menuText(MENU_LABEL_X, MENU_TARGET_Y, fxfontg, MENU_LBL_TARGET, 6);
    menuOptionRow(MENU_TARGET_NAMES, MENU_TARGET_OFF, MENU_TARGET_LEN, 4, m.target, MENU_TARGET_Y);
